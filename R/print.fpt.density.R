...
function (x, ...) 
{
    ...
    cat(paste("\n   $x: a sequence of", length(x$x), "time instants from", 
        ifelse(t0 < x$x[1], format(x$x[1], ...), t0), "to", ifelse(x$x[length(x$x)] < 
            T, format(x$x[length(x$x)], ...), T), sep = " "))
    ...
    if (!cp) 
        label <- c(label, "Number of equally spaced values considered in the range of the initial distribution")
    if (logic) 
        label <- c(label, "Maximum slope required between two points to consider that a growing function is constant", 
    ...
    values <- as.character(c(Args[c("variableStep", "from.t0", 
        "to.T", "skip")], 1 - Args$tol))
    if (!cp) 
        values <- c(values, as.character(Args$m))
    if (logic) 
        values <- c(values, as.character(Args$zeroSlope), paste("10^(-", 
            Args$p0.tol, ")", sep = ""), "", paste(Args$k, "*(tmax[i] - t[i]*)(1-FPTL(tmax[i]))", 
            sep = ""))
    values <- c(values, paste(Args$n, "/(tmax[i]^- - t[i]*)", 
        sep = ""), paste(Args$p, "*", Args$n, "/(tmax[i]^- - t[i]*)", 
        sep = ""), "", ifelse(Args$alpha == 1L, paste(Args$p, 
        "*", Args$n, "/(tmax[i]^- - t[i]*)", sep = ""), paste(Args$p, 
        "*", Args$n, "*(t[i+1]* - tmax[i]^+)^(", Args$alpha - 
            1L, ")/(tmax[i]^- - t[i]*)^", Args$alpha, sep = "")), 
        "")
    ...
    ut <- format(sum(y[, 5]), ...)
    y <- format(y, ...)
    y[, 1] <- paste("(", format(lower, ...), ", ", format(upper, 
        ...), "]", sep = "")
    cat("\nApplication summary of the numerical algorithm:\n\n")
    y <- rbind(c("Subinterval", "Integration step", "Cumulative integral", 
        "Iterations", "User time"), y)
    y <- apply(y, 2, format, justify = "right")
    cat(apply(y, 1, paste, collapse = "  "), sep = "\n")
    label <- paste(format(c("Total number of iterations", "Total user time")), 
        "   ", c(it, ut))
    cat("\n")
    cat(label, sep = "\n")
