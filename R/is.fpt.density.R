...
function (obj) 
{
    if (inherits(obj, "fpt.density") & is.list(obj) & (length(obj) == 
        3L)) {
        if (all(sapply(obj[1:2], is.numeric)) & all(is.element(names(obj), 
            c("x", "y", "y.x0"))) & all(is.element(names(attributes(obj)), 
            c("names", "Call", "Steps", "cumIntegral", "skips", 
                "CPUTime", "summary.fptl", "class")))) {
            if (is.vector(obj$x) & is.vector(obj$y) & is.call(attr(obj, 
                ...
                is.list(attr(obj, "skips")) & is.matrix(attr(obj, 
                "CPUTime")) & is.numeric(attr(obj, "CPUTime"))) {
                if ((length(obj$x) == length(obj$y)) & all(diff(attr(obj, 
                  "cumIntegral")) >= 0) & (length(attr(obj, "cumIntegral")) == 
                  length(attr(obj, "skips"))) & (length(attr(obj, 
                  "cumIntegral")) == nrow(attr(obj, "CPUTime"))) & 
                  (nrow(attr(obj, "Steps")) >= nrow(attr(obj, 
                    "CPUTime"))) & (ncol(attr(obj, "Steps")) == 
                  3) & (ncol(attr(obj, "CPUTime")) == 2) & all(sapply(attr(obj, 
                  "skips"), is.integer))) {
                    ...
                    if (f == "Approx.fpt.density") {
                      if (!is.null(obj$y.x0)) {
                        if (is.numeric(obj$y.x0) & is.matrix(obj$y.x0)) {
                          if ((length(obj$x) != nrow(obj$y.x0)) | 
                            (ncol(obj$y.x0) != (length(attr(obj, 
                              "summary.fptl")))) | any(obj$y.x0 < 
                            0L)) 
                            return(FALSE)
                        }
                        else return(FALSE)
                      }
                      if (is.numeric(Args$id)) 
                        m <- 1L
                      else {
                        m <- Args$m
                        if (is.null(m)) 
