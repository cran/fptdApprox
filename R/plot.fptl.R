    ...
    ...) 
{
    ...
    if (dp.legend) {
        ...
        dpMean <- eval(parse(text = paste("substitute(", attr(x, 
            "dp")$mean, ", Args[logic])", sep = "")))
        dpVar <- eval(parse(text = paste("substitute(", attr(x, 
            "dp")$var, ", Args[logic])", sep = "")))
    ...
    if (!to.T) {
        if (x$x[length(x$x)] > FPTLInstants[nrow(FPTLInstants), 
            5]) {
            j <- which(x$x >= FPTLInstants[nrow(FPTLInstants), 
                5])[1]
    ...
    if (growth.points) {
        segments(FPTLInstants[, 1], par("usr")[3], FPTLInstants[, 
            1], y3, col = "darkgray", lwd = 1)
        text(FPTLInstants[, 1] - 0.1 * par("cxy")[1], y3 - 0.5 * 
            strheight(expression(t), cex = sqrt(par("cex"))) - 
            0.1 * par("cxy")[2], parse(text = paste("t[~ ", 1:nrow(FPTLInstants), 
            "]", sep = "")), adj = 1, col = "darkgray")
    }
    if (instants) {
        ...
        text(FPTLInstants[, 2] - 0.1 * par("cxy")[1], -0.65 * 
            strheight(expression(t[1]), cex = sqrt(par("cex"))) - 
            0.1 * par("cxy")[2], parse(text = paste("t[~ ", 1:nrow(x.t), 
            "]^{~ ", shQuote("*"), "}", sep = "")), adj = 1)
        mtext(parse(text = paste("t[list(~ max,", 1:nrow(FPTLInstants), 
            ")]^{~ ", shQuote("-"), "}", sep = "")), side = 1, 
            line = 1.6, at = FPTLInstants[, 3], adj = 0, ...)
        mtext(parse(text = paste("t[list(~ max,", 1:nrow(FPTLInstants), 
            ")]^{~ ", shQuote("+"), "}", sep = "")), side = 1, 
            line = 3, at = FPTLInstants[, 5], adj = 0, ...)
    }
}
